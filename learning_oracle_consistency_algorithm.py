...
def learning_oracle_consistency_algorithm(x_test, y_pred, y_test, alpha):
  ...
  for idx in range(len(y_test_resid_pos)):
    if (y_test_resid_pos[idx] > .5):
      pos_high_error_set_indices.append(idx)
    if (y_test_resid_neg[idx] > .5):
      neg_high_error_set_indices.append(idx)

  return pos_high_error_set_indices, neg_high_error_set_indices, beta1, beta2
